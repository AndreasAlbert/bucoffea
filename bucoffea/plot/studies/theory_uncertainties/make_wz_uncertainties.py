...
def from_coffea(inpath, outfile):
    ...
    for year in [2017,2018]:
        ...
        # EWK V
        h_z = acc['mjj'][re.compile(f'.*EWK_Z.*{year}')].integrate('dataset')
        f[f'z_ewk_mjj_nominal_{year}'] = export1d(h_z)

        h_w = acc['mjj'][re.compile(f'.*EWK_W.*{year}')].integrate('dataset')
        f[f'w_ewk_mjj_nominal_{year}'] = export1d(h_w)

        # EWK Variations
        h_z_unc = acc['mjj_unc'][re.compile(f'.*EWK_Z.*{year}')].integrate('dataset')
        for unc in map(str, h_z_unc.axis('uncertainty').identifiers()):
            h = h_z_unc.integrate(h_z_unc.axis('uncertainty'), unc)
...
def main():
    ...
    if not os.path.exists(outdir):
        os.makedirs(outdir)
    inpath = './input/2019-11-21_vbf_theory_unc_v4'
